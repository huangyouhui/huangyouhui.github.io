...
`seata使用记忆`: 在事务发起方的方法入口处加入@Gl的注解，开启全局事务，事务开启后会生成一个全局事务ID，当当前事务执行过程中遇到update,insert操作的时候
会往seata的undo_log表中记录改条记录的前置镜像log,当执行update，insert操作后会记录该条记录的后置镜像记录log，并且会直接提交本地事务。此为第一阶段提交。
在记录log日志的同事，该分支事务会向TC记录分支事务记录。当所有分支事务都第一阶段提交后，并且中间没有任何异常的情况下，直接提交完成 删除所有undo_log事务日志
和分支事务全局事务记录信息。当任何一个分支事务提交失败的情况下 ，则进行第二阶段提交，TC会通知所有分支进行二阶段提交回滚通过undo_log的前置镜像进行二阶段提交回滚。

`seata的使用非常简单，只需要在事务发起者加上注解`

seata提供的几种模式：AT、TCC、SAGA、XA
